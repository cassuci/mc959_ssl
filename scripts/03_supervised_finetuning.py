...
def train_model(model, train_dataset, val_dataset, epochs=10):
    ...
    checkpoint = tf.keras.callbacks.ModelCheckpoint(
        "models/finetuning_resnet18_mae/baseline_{epoch:02d}_{val_loss:.2f}.h5",
        monitor="val_loss",
        verbose=1,
        save_best_only=True,
...
if __name__ == "__main__":
    data_path = "/mnt/f/ssl_images/data"  # "/mnt/f/ssl_images/data" if you're Gabriel
    data_dir = os.path.join(data_path, "processed", "pascal_voc")
    metadata_dir = os.path.join(data_path, "pascal_voc", "ImageSets", "Main")
    pretrained_model = os.path.join(
        "models", "checkpoints_resnet18_mae", "best_model.h5"
    )  # path to pretrained model if it's finetuning
    ...
    save_path = os.path.join("models", "finetuning_resnet18_mae.h5")
