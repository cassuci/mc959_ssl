...
def generateRandomMask(mask_size: Tuple[int, int], img_size: Tuple[int, int], n_masks: int):
...
def get_dataset(
    ...
    for img in os.listdir(dataset_path):
        ...
        # Insert batch axis
        input_image = tf.expand_dims(tf.cast(input_image, tf.float32) / 255.0, axis=0)
        gt_image = tf.expand_dims(tf.cast(gt_image, tf.float32) / 255.0, axis=-1)
