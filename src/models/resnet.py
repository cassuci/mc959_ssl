...
class ResNet(BaseModel):
    ...
    def build(self, input_shape):
        ...
        for i, skip in enumerate(reversed(skip_outputs)):
            x = tf.keras.layers.Conv2DTranspose(x.shape[-1] // 2, 3, strides=2, padding="same")(
                x
            )  # Upsample
            x = tf.keras.layers.BatchNormalization()(x)
            x = tf.keras.layers.ReLU()(x)
        ...
        x = tf.keras.layers.Conv2DTranspose(128, kernel_size=3, strides=2, padding="same")(
            x
        )  # -> 448x448
        x = tf.keras.layers.BatchNormalization()(x)
        x = tf.keras.layers.ReLU()(x)
        ...
        outputs = tf.keras.layers.Conv2D(3, kernel_size=3, padding="same", activation="sigmoid")(
            x
        )  # Final output: 224x224x3
